class LearningSpace {
    ...
    saveWidgetPositions() {
        this.widgets.forEach(widget => {
            ...
            if (typeof widget.updatePosition === 'function') {
                widget.updatePosition(widget.position);
            }
        });
        this.saveLayout();
    }
    ...
    saveLayout() {
        // Find the space in the main app and update its widgets
        const space = window.classHub.spaces.find(s => s.id === this.id);
        if (space) {
            // Create a deep copy of the widgets array with necessary data
